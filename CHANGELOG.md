...
- **源码通道**基于 deepseek-harness 源码 `0.1.2-rc.1`（file: 本地解析，246 包）
- **包变化**：248 → 246（-2）；新增 0 包 / 移除 2 包 / 版本变更 236 个
- **应用版本**：`0.2.1` → `0.2.1-rc.1`
**提交信息（模板）**
```bash
git add -A && git commit -m "build(deps): upgrade deepseek-harness to 0.1.2-rc.1 (source) and app to 0.2.1-rc.1"
```
**升级要点**
